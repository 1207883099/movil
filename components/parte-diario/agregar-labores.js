...
export function AgregarLabores({ setLaboresAsignado }){
    ...
    const obtener_empleado = (IdEmpleado) => {
        const result_empleado = Empleados.find(empleado => empleado.IdEmpleado === IdEmpleado);
    ...
    const guardar_integrantes = () => {
        ...
        if(SelectEmpleado && Labor){
            if(obtener_empleado_asignado(SelectEmpleado) == undefined){
                ...
                Alert.alert(`${obtener_empleado(SelectEmpleado)} ya fue guardado anteriormente.`);
            }
        }else{
            Alert.alert(`Seleccione algun empleado de su cuadrilla: ( ${IsCuadrilla} )`);
        }
    }
    ...
    return(
        <>
                    ...
                    <View style={styles.select}>
                        ...
                        >
                            {Labores.map((labor, index) => (
                                <Picker.Item key={index} label={labor.Nombre} value={labor.IdLabor} />
                            ))}
                        </Picker>
                    </View>
                </>
            ...
            {IsCuadrilla ? (
                    ...
                    <View style={styles.select}>
                        ...
                        >
                            {Empleados.map((empleado, index) => (
                                <Picker.Item key={index} label={empleado.Nombre + ' - ' + empleado.Apellido} value={empleado.IdEmpleado} />
                            ))}
                        </Picker>
                    </View>
                </>
                ...
                <>
                    <Text style={[styles.label, {textAlign: 'center'}]}>Seleccione la cuadrilla</Text>
                    ...
                    <View style={styles.select}>
                        <Picker
                            ...
                            onValueChange={itemValue => {
                                if(itemValue !== 'none'){
                                    ...
                                    const ResultEmpleados = Cuadrillas.find(cuadrilla => cuadrilla.Nombre === itemValue);
                        ...
                        >
                            ...
                            {Cuadrillas.map((cuadrilla, index) => (
                                <Picker.Item key={index} label={cuadrilla.Nombre} value={cuadrilla.Nombre} />
                            ))}
                        </Picker>
                    </View>
                </>
            ...
            {SelectEmpleado && <Button title={`Guarda obrero en: ( ${obtener_labor(Labor)} )`} onPress={guardar_integrantes} />}
            <Text>{'\n'}</Text>
            {Asignado.length > 0 && <Button style={{ marginTop: 20 }} color='green' title={`Finalizar asignaciones`} onPress={finalizar_asignaciones} />}
        </>
    );
}
